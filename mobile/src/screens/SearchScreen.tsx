...
export function SearchScreen({navigation, route}: Props) {
  ...
  const [listingType, setListingType] = useState<'sale' | 'rent' | ''>(
    route.params?.listingType === 'sale' || route.params?.listingType === 'rent'
  ...
  const buildFilters = useCallback((): PropertyFilters => {
    const filters: PropertyFilters = {limit: 100};
    ...
    query, location, state, propertyType, listingType, minPrice, maxPrice,
  ...
  // Live search (debounced) when keyword / listing / type / location change
  useEffect(() => {
    const hasSignal =
      query.trim().length >= 2 ||
      !!location.trim() ||
  ...
  }, [query, location, state, propertyType, listingType, handleSearch]);

  useEffect(() => {
    if (route.params?.location || route.params?.propertyType || route.params?.listingType) {
      const run = async () => {
        setLoading(true);
        setSearched(true);
        try {
          ...
          if (route.params?.listingType) filters.listingType = route.params.listingType;
  ...
  }, [route.params?.location, route.params?.propertyType, route.params?.listingType]);
    ...
    <ScreenShell title={t('search.title')} subtitle={t('search.subtitle')} tabScreen>
      <View style={styles.container}>
        <ScrollView
          ...
          <View style={styles.card}>
            <AppText variant="h3" style={styles.cardTitle}>Type d'annonce</AppText>
        ...
        {searched && (
          <View style={styles.resultsWrap}>
            ...
            <FlatList
              ...
              ListEmptyComponent={
                <EmptyState
                  message="Aucun bien ne correspond à vos critères"
...
const styles = StyleSheet.create({
  ...
  resultsContent: {padding: 16, paddingTop: 0, paddingBottom: 32},
});
