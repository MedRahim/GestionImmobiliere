...
const normalizeInquiry = (row) => ({
  ...
  propertyTitle: row.PropertyTitle || 'Annonce supprimée',
  ...
  propertyDeleted: row.PropertyDeletedAt != null || row.PropertyIsActive === 0 || row.PropertyIsActive === false,
});
  ...
  try {
    ...
    const result = await query(
      `SELECT i.InquiryId, i.PropertyId, i.ClientId, i.AgentId, i.Subject, i.Message,
              ...
              p.DeletedAt AS PropertyDeletedAt,
       ...
       LEFT JOIN [dbo].[Properties] p ON p.PropertyId = i.PropertyId
...
exports.createInquiry = async (req, res, next) => {
  try {
    ...
    const clientFirst = (req.user.firstName || '').trim() || 'Un acheteur';
    const clientName = `${req.user.firstName || ''} ${req.user.lastName || ''}`.trim() || clientFirst;
    await createNotification({
      userId: property.AgentId,
      type: 'inquiry',
      title: `${clientFirst} s'intéresse à votre bien`,
      message: `"${property.Title}" — ouvrez pour répondre`,
