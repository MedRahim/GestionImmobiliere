...
/** Casual chat — never dump listings for "ça va", merci, etc. */
exports.trySmallTalk = (question, locale = 'fr') => {
  const raw = String(question || '').trim();
  if (!raw || raw.length > 120) return null;
  const q = norm(raw);
  const fr = locale !== 'en';

  // "ça va" alone OR mixed with greeting: "bonjour comment ca va", "salut ca va ?"
  const asksHowAreYou =
    /\b(ca va|cava|sa va|comment ca va|comment vas[- ]tu|how are you|how r you|whats up|what'?s up)\b/i.test(
      q,
    ) ||
    /\b(ça va|comment ça va)\b/i.test(raw);

  if (asksHowAreYou && !/publier|prix|bien|appart|quartier|securit/.test(q)) {
    return fr
      ? `Ça va très bien, merci ! Je suis le guide Immo Dary — prêt à vous aider sur un bien, un quartier, les prix ou l'application. Que puis-je faire pour vous ?`
      : `I'm doing great, thanks! I'm the Immo Dary guide — properties, areas, prices, or the app. How can I help?`;
  }

  if (/^(merci|thanks|thank you|thx|merci beaucoup|chokran)([\s!?.]*)?$/i.test(q)) {
    return fr
      ? `Avec plaisir ! Posez-moi une autre question quand vous voulez.`
      : `You're welcome! Ask anytime.`;
  }

  if (/^(ok|okay|daccord|d'accord|parfait|super|cool|top|yes|oui|non|np)([\s!?.]*)?$/i.test(q)) {
    return fr
      ? `Parfait. Dites-moi ce dont vous avez besoin — un bien, un quartier, ou de l'aide sur l'app.`
      : `Got it. What would you like help with?`;
  }

  if (
    /^(bye|au revoir|a plus|a bientot|ciao|bonne journee|bonne soiree|bonne chance)([\s!?.]*)?$/i.test(q) ||
    /^(à plus|à bientôt|bonne journée|bonne soirée)([\s!?.]*)?$/i.test(raw)
  ) {
    return fr ? `À bientôt ! Bonne recherche sur Immo Dary.` : `See you soon!`;
  }

  if (/^(qui es[- ]tu|t'?es qui|who are you|c'?est quoi ton role)([\s!?.]*)?$/i.test(q)) {
    return fr
      ? `Je suis le guide IA d'Immo Dary. Je réponds sur les annonces, quartiers, proximité, sécurité, prix, et comment utiliser l'app. Vous pouvez aussi m'envoyer une photo ou un PDF.`
      : `I'm Immo Dary's AI guide for listings, areas, and the app. You can also send photos or PDFs.`;
  }
...
exports.tryNavigationInstant = (question, _context = {}, locale = 'fr') => {
  ...
  const small = exports.trySmallTalk(raw, locale);
  if (small) return small;

  // Intent answers first — don't swallow "Bonjour, comment publier… ?"
  ...
  if (/utiliser l.?application|navigation|comment ca marche|comment ça marche|comment fonctionne/.test(q) && !/quartier|lieu|ville/.test(q)) {
    return fr
      ? `Voici comment Immo Dary fonctionne :\n\n• Accueil : carte + liste (glissez la feuille du bas)\n• Loupe (en haut) : recherche avancée — ce n’est PAS un onglet du bas\n• Favoris / Messages : onglets du bas (connexion requise)\n• Guide : onglet central pour me poser des questions\n• Profil : compte & langue\n• Menu ☰ : Mes annonces, Demandes, Publier\n• Publier : bouton + turquoise sur la carte`
      : `• Home: map + list\n• Header search icon (not a bottom tab)\n• Favorites / Messages / Guide / Profile tabs\n• Menu: listings, inquiries, publish\n• Publish: + button on the map`;
  }
  ...
  // Greeting only when the message is basically just hello
  if (/^(bonjour|salut|hello|hi|coucou|bonsoir)([\s,!.؟?]*)?$/i.test(raw)) {
    return fr
...
exports.synthesizeResearchReply = (question, research, context, locale = 'fr') => {
  ...
  const userPosLine = userLoc?.latitude
    ? `\n📍 Position GPS de l'utilisateur : ${userLoc.latitude.toFixed(5)}, ${userLoc.longitude.toFixed(5)}` +
      (userLoc.accuracy != null ? ` (±${Math.round(userLoc.accuracy)} m)` : '') +
      (userLoc.address ? `\nAdresse approximative : ${userLoc.address}` : '') +
      `\nUtilise ces infos pour les distances et le quartier.`
    : '';
...
exports.composeGeneralReply = (question, research, context, locale = 'fr') => {
  ...
  // Only suggest nearby listings when the user actually asks about properties / proximity
  const wantsListings =
    /proche|pres|autour|near|recommand|suggest|bien|appart|villa|maison|cherche|trouver|annonce|budget|prix/.test(
      q,
    );
  if (wantsListings && research.nearUserListings?.length > 0) {
    return fr
      ? `**Autour de vous :**\n${research.nearUserListings
          .slice(0, 4)
  ...
  // Never dump random sample listings — only when the user clearly wants properties
  if (wantsListings && context.sampleProperties?.length > 0) {
    return fr
      ? `Voici quelques annonces sur Immo Dary :\n${context.sampleProperties
          .slice(0, 4)
          .map((p) => `• ${p.title} (${p.city}) — ${Number(p.price).toLocaleString('fr-TN')} TND`)
          .join('\n')}\n\nPrécisez une ville, un budget ou « biens proches de moi ».`
      : `Here are some listings. Refine city or budget.`;
  }

  return fr
    ? `Je suis le guide Immo Dary. Exemples :\n• « Ce quartier est-il sûr ? »\n• « Biens proches de moi »\n• « Prix moyen à Sousse »\n• « Comment contacter le vendeur ? »\n• Ou envoyez une photo / PDF avec le trombone 📎`
    : `Ask about areas, prices, or properties — or attach a photo/PDF.`;
};
...
- Answer the user's ACTUAL question directly — match their intent and tone.
- Casual chat ("ça va", "merci", "ok"): answer warmly as a human; NEVER dump random listings.
- Only mention listings when the user asks about properties, prices, areas, recommendations, or proximity.
- Be natural, concrete and useful — short paragraphs or bullets (max ~12 sentences).
- For property/city/safety/prices/proximity: use RESEARCH DATA (do not invent listings or distances).
- For "how to use the app" questions: follow APP HELP exactly — never invent menus or tabs.
- If the user attached an image or document: analyze it in a real-estate / Immo Dary context and answer helpfully.
- CRITICAL UI FACTS:
  • Bottom tabs are ONLY: Accueil (map), Favoris, Guide, Messages, Profil.
  • There is NO "Recherche" tab at the bottom. Search = magnifying-glass icon in the top header.
  • Guide = center bottom tab (not a floating bubble).
  • Publish = turquoise + button on the map home screen, or Menu → Publier.
- Never say "reformulez votre question" unless truly stuck.
- If GPS/user location is missing for a distance question, say so and explain how to enable it.
